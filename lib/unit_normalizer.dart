...
class UnitNormalizer {
  ...
  static String? convert(String? raw) {
    final normalized =
        TextNormalizer.normalize(raw)?.toLowerCase();
    if (normalized == null) {
      return null;
    }
  ...
  static bool quantitiesEquivalent({
    ...
    if (convertedExpected == null || convertedActual == null) {
      return false;
    }
    if (convertedExpected.dimension != convertedActual.dimension) {
      return false;
    }

    final tolerance =
        (convertedExpected.value.abs() * 0.001).clamp(0.01, 1000.0);
    return (convertedExpected.value - convertedActual.value).abs() <=
        tolerance;
  }
}

  ...
  group('IdGenerator.uuidV4', () {
    test('generates a valid UUID v4 format', () {
      ...
      expect(
        pattern.hasMatch(id),
        isTrue,
        reason: 'Expected UUID v4 format: $id',
      );
    });

    test('generates unique IDs across multiple calls', () {
      final ids = <String>{for (var i = 0; i < 100; i++) IdGenerator.uuidV4()};
  ...
  group('IdGenerator.prefixed', () {
    test('prepends the given prefix followed by a hyphen', () {
      ...
      expect(
        id.startsWith('quote-'),
        isTrue,
        reason: 'Expected quote- prefix: $id',
      );
    });
    ...
    test('the part after prefix is a valid UUID v4', () {
      ...
      expect(
        pattern.hasMatch(uuidPart),
        isTrue,
        reason: 'Expected UUID v4 after prefix: $uuidPart',
      );
    });
  });
}
